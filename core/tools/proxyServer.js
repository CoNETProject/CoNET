...
exports.checkDomainInBlackList = (BlackLisk, domain, CallBack) => {
    if (Net.isIP(domain)) {
...
exports.tryConnectHost = (hostname, hostIp, port, data, clientSocket, isSSLConnect, checkAgainTimeOut, connectTimeOut, gateway, CallBack) => {
    if (isSSLConnect) {
        clientSocket.once('data', (_data) => {
            return exports.tryConnectHost(hostname, hostIp, port, _data, clientSocket, false, checkAgainTimeOut, connectTimeOut, gateway, CallBack);
        });
...
exports.isAllBlackedByFireWall = (hostName, ip6, gatway, userAgent, domainListPool, CallBack) => {
    ...
    return exports.checkDomainInBlackList(blackDomainList, hostName, (err, result) => {
        ...
        if (!hostIp && !this.useGatWay) {
            return exports.isAllBlackedByFireWall(hostName, ip6, gatway, userAgent, domainListPool, (err, _hostIp) => {
                ...
                return exports.tryConnectHost(hostName, _hostIp, port, buffer, clientSocket, httpHead.isConnect, checkAgainTime, connectTimeOut, useGatWay, CallBack);
            });
        }
        return exports.tryConnectHost(hostName, hostIp, port, buffer, clientSocket, httpHead.isConnect, checkAgainTime, connectTimeOut, useGatWay, CallBack);
    });
};
/*
