...
const _appScript = {
		...
		return _view.keyPairCalss.emitRequest ( com, ( err, com: QTGateAPIRequestCommand ) => {
			...
			return _view.connectInformationMessage.sockEmit ( 'getFilesFromImap', arg, ( err, buffer: string ) => {
				...
				return _view.keyPairCalss.decryptMessageToZipStream ( buffer, ( err, data ) => {
					...
					let y = null
					self.showWebPage ( y = new self.showWebPageClass ( search_text, buffer, uuid , () => {
	...
	showSnapshotClick: ( self, index ) => {
		...
		self.showWebPage ( y = new self.showWebPageClass ( currentItem.url, currentItem.snapshotData, currentItem.snapshotUuid , () => {
			self.showWebPage ( y = null )
			self.showMain ( true )
			self.showSnapshop ( false )
	...
	imageSearch: ( ee ) => {
			...
			reader.onload = e => {
				...
				return self.getPictureBase64MaxSize_mediaData ( rawData, 1024, 1024, ( err, data ) => {
					...
					return _view.connectInformationMessage.sockEmit ('sendMedia', uuid, data.rawData, err => {
...
class appsManager {
	...
	*/
	...
	private updateAppsManu ( appMenu ) {
		const self = this
		...
		*/
		...
		/**
		 * 
		 * 			DEBUG use temp script to build APPs
		 * 
		 */
		/*
		setTimeout (() => {
