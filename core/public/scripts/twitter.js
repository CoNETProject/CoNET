...
const splitLine = function (user, fullText, entities) {
    if (user && user.length) {
...
const getTimeFromCreate = function (created_at, twitter) {
    const now = new Date();
...
const convertSvgToPng = function (svgUrlData, CallBack) {
    let image = new Image();
...
function parseFile(process, item, file, CallBack) {
...
var twitter_layout;
(function (twitter_layout) {
    class twitterField {
        constructor(twitter) {
            this.twitter = twitter;
            this.inputText = ko.observable('');
            this.textAreaHeight = ko.observable(3);
            this.stroke_dashoffset = ko.observable(initDashoffset);
            this.stroke_dashoffset_showSafe = ko.observable(true);
            this.stroke_dashoffset_showDanger = ko.observable(false);
            this.avaliableText = ko.observable(twitterMaxTextCount);
            ...
            this.videoSize = 0;
            this.twitterLength = 0;
            this.lastTextlength = 0;
            this.textAreaError = ko.observable(false);
            this.newTwitterFieldError = ko.observable(false);
            this.fileReadError = ko.observable(false);
            this.uploadVideo = ko.observable(false);
            const self = this;
            this.inputText.subscribe(function (ns) {
                ...
                if (self.lastTextlength < ns.length) {
                    if (ns.length - self.lastTextlength > 1 || / $/.test(ns)) {
                        return socketIo.emit11('getTwitterTextLength', ns, function (twTextObj) {
                            if (!twTextObj.valid) {
    ...
    twitter_layout.twitterField = twitterField;
    class twitter {
        constructor() {
            //- for add new Twitter account
            this.apiKey = ko.observable('');
            this.apiKeyError = ko.observable(false);
            this.apiSecret = ko.observable('');
            this.apiSecretError = ko.observable(false);
            this.accessToken = ko.observable('');
            this.accessTokenError = ko.observable(false);
            this.accessTokenSecret = ko.observable('');
            this.accessTokenSecretError = ko.observable(false);
            this.showAddTwitterAccount = ko.observable(false);
            //-
            ...
            this.showServerError = ko.observable(false);
            this.password = ko.observable('');
            this.passwordError = ko.observable(false);
            this.showLoader = ko.observable(false);
            this.showAccountMenu = ko.observable(false);
            this.twitterData = ko.observableArray([]);
            this.currentTwitterAccount = ko.observable();
            this.currentTimelines = ko.observableArray([]);
            this.showCurrentTimelines = ko.observable(false);
            this.showAccountError = ko.observable(false);
            this.newTwitterField = ko.observableArray([]);
            this.addButtonDisabled = ko.observable(true);
            this.showDistroynewTwitter = ko.observable(true);
            this.shownewTwitterApprove = ko.observable(false);
            ...
            this.bottomEventLoader = ko.observable(false);
            ...
            socketIo.once('connect', function () {
                return socketIo.emit11('init', function (err, data) {
                    if (!data) {
        ...
        login() {
            ...
            return socketIo.emit11('password', this.password(), function (err, data) {
                if (err) {
        ...
        showCurrentTwitterAppInfomation() {
            this.resetAddAccountError();
    ...
    twitter_layout.twitter = twitter;
